...
# set directories
toolPATH='./'
tssFilesPATH='../01_additionalData/TSSfiles/'
picardPATH='/home/wjg/Applications/picard-tools-1.77/'
...
do
	...
	out3=`echo $out1 | sed 's/.bam/.st/'`
        ...
        then
            ...
            echo "Sorting: " $out1
            java -jar -Djava.io.tmpdir=`pwd`/tmp $picardPATH/SortSam.jar SO=coordinate I=$out1 O=$out3.bam VALIDATION_STRINGENCY=SILENT
	...
	then
	    ...
	    java -jar -Djava.io.tmpdir=`pwd`/tmp $picardPATH/MarkDuplicates.jar INPUT=temp.bam OUTPUT=$out6 METRICS_FILE=$out5 REMOVE_DUPLICATES=true VALIDATION_STRINGENCY=SILENT
            samtools index $out6
	    rm temp.bam
	fi
	...
	then
	    ...
	    # get insert-sizes
	    echo '' > $out8
	    java -jar $picardPATH/CollectInsertSizeMetrics.jar VALIDATION_STRINGENCY=SILENT I=$out6 O=$out8 H=$out9 W=1000	
	fi
