...
# The following program will extract relevent info from a bw file
...
# dups log
cmd1='find '+options.d+' -name "*.dups.log" | sort -V > ReadDups'
os.system(cmd1);
dupsFiles = np.loadtxt('ReadDups','str')

# get list of align stats data
cmd2='find '+options.d+' -name "*.stats.log" | sort -V > ReadStats'
os.system(cmd2);
statFiles = np.loadtxt('ReadStats','str')

# get iSizes
cmd3='find '+options.d+' -name "*.hist_data.log" | sort -V > ReadiSizes'
os.system(cmd3);
iSizeFiles = np.loadtxt('ReadiSizes','str')

# get enrich info
cmd4='find '+options.d+' -name "*.RefSeqTSS" | sort -V > ReadTSS'
os.system(cmd4);
tssFiles = np.loadtxt('ReadTSS','str')

# get alignment info
cmd5='find '+options.d+' -name "*.align.log" | sort -V > ReadAlignInfo'
os.system(cmd5);
...
# loop and cat data
for i in range(0,len(dupsFiles)):
    # name
    ...
    for i in range(0,len(iSizeData)):
        k = iSizeData[i]
        outIsize[int(k.split('\t')[0])-1] = int(k.split('\t')[1])
